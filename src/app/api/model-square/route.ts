...
function compareModelName(left: SquareModel, right: SquareModel, order: string) {
  const result = (left.Name ?? "").localeCompare(right.Name ?? "")

  return order === "Desc" ? -result : result
}

function compareModelTime(left: SquareModel, right: SquareModel, order: string) {
...
async function fetchModelPage({
  ...
  return callUCloudAction<ListUFSquareModelResponse>({
    credentials,
    params,
  })
}
...
async function fetchAllModels({
  ...
  const firstPage = await fetchModelPage({
    credentials,
    projectId,
    offset: 0,
...
function toErrorResponse(error: unknown) {
  ...
  return NextResponse.json(
    {
      ok: false,
      message: "Unexpected model square request failure.",
    },
    { status: 500 }
  )
}
